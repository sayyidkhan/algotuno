...
function get_training_and_labelled_data_from_stock(_stock_data, _year, _window) {
    // data for processing
    let training_data = get_stock_data(_stock_data, _year, _window); // training data is [13, 4]
    let labelled_data = get_labeled_data(training_data);
    ...
    labelled_data = convert_label_data_into_list(labelled_data);
    // 4. remove dates
    let remove_date_training_data = training_data.map(x => x.map(y => y[0]));
    let remove_data_labelled_data = labelled_data.map(x => [x[0]]);
...
async function forecast(_model, _training_data) {
    const _forecast = await _model.predict(_training_data);
    console.log("perform forecasting");
    return _forecast;
}

export {
    get_training_and_labelled_data_from_stock,
    nn_model,
    train,
    predict,
    forecast,
